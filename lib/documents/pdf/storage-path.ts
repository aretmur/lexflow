...
export function signedAgreementStoragePath(input: {
  firmId: string;
  agreementId: string;
  versionNumber: number;
}): string {
  if (!Number.isInteger(input.versionNumber) || input.versionNumber < 1) {
    throw new Error("Signed agreement version must be a positive integer");
  }
  return `${input.firmId}/${input.agreementId}/version-${input.versionNumber}/signed-agreement.pdf`;
}
